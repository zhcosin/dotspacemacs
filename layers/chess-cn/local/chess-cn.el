...
(defun chess-cn--put-piece-to-board (row col row-situation board-row-str)
  ...
  (let* ((board-row-str-len (length board-row-str))
         ...
         (last-history (chess-cn--top-history)))
    (if (< col (length row-situation))
        (let ((curt-piece (aref row-situation col)))
          (concat
           (if
               curt-piece ;; 有棋子
               (concat
                (propertize (chess-cn--get-piece-name curt-piece) 'font-lock-face
                            (cond
                             (is-selected-piece (chess-cn--get-piece-selected-face curt-piece))
                             ((and last-history
                                   (equal row (cdr (plist-get last-history 'dstcord)))
...
(defun chess-cn--move-piece (oldcord dstcord)
  ...
  (let ((moved-piece (chess-cn--get-piece-from-situation oldcord)))
    (chess-cn--move-piece-impl moved-piece oldcord dstcord)
    (plist-put chess-cn--playing 'curt-selected-cord nil)
...
(defun chess-cn--push-history (oldcord dstcord kill-piece killed-piece)
  "记录棋步历史,killed-piece 为被吃子(符号)"
  (plist-put chess-cn--playing
             'history
             (cons (list 'oldcord oldcord 'dstcord dstcord 'kill-piece kill-piece 'killed-piece killed-piece)
...
(defun chess-cn--top-history()
  "获取最后一步棋步历史"
  (car (plist-get chess-cn--playing 'history)))
